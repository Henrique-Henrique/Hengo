...
func test_vcnode_io() -> void:
	...
	assert_int((global.SAVE_DATA.get_connection_from_vc(vc)[0] as HenVCConnectionData).get_from().identity.id).is_equal(vc2.identity.id)
	assert_int((global.SAVE_DATA.get_connection_from_vc(vc)[0] as HenVCConnectionData).get_to().identity.id).is_equal(vc.identity.id)
...
func test_vcnode_flow() -> void:
	...
	assert_bool((global.SAVE_DATA.get_flow_connection_from_vc(vc)[0] as HenVCFlowConnectionData).get_from().identity.id == vc.identity.id).is_true()
