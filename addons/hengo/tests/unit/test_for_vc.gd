...
func test_for_range_code() -> void:
	...
	# test base
	assert_eq(
		HenTest.construct_and_get_code(for_vc, [], refs),
		'for index_2 in range(0, 0, 1):\n\tpass'
	)
	...
	# test id connection
	assert_eq(
		HenTest.construct_and_get_code(for_vc, [vc_flow_void], refs),
		'for index_2 in range(0, 0, 1):\n\ttest_void(index_2)'
	)
	...
	# test body flow
	assert_eq(
		HenTest.construct_and_get_code(for_vc, [vc_flow_1], refs),
		'for index_2 in range(0, 0, 1):\n\ttest_void()'
	)
	...
	# test then flow
	assert_eq(
		HenTest.construct_and_get_code(for_vc, [vc_flow_1, vc_flow_2], refs),
		'for index_2 in range(0, 0, 1):\n\ttest_void()\ntest_void()'
	)
