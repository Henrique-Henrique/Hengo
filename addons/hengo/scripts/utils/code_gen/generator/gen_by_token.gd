...
static func get_code_by_token(_save_data: HenSaveData, _token: Dictionary, _level: int = 0, _parent_id: String = '', _inline: bool = false) -> String:
	...
	match _token.type as HenVirtualCNode.SubType:
		...
		HenVirtualCNode.SubType.NOT_CONNECTED:
			return HenVirtualCNodeCode.get_default_value_code(_save_data, _token.input_type, use_self, _token.get('category', ''), _token.get('data', null), _token.get('value', null))
		...
		HenVirtualCNode.SubType.RAW_CODE:
			var raw_text: String = (_token.code.value as String).trim_prefix('"').trim_suffix('"').strip_edges(false, true)
