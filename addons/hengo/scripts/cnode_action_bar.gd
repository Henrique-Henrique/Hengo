...
func _ready() -> void:
    get_node('Left/Open').pressed.connect(_on_open_press)
    get_node('Left/Create').pressed.connect(_on_create_press)
...
func _on_open_press() -> void:
    ...
    Global.GENERAL_POPUP.get_parent().show_content(list, 'Open Script', get_node('Left/Open').global_position)
