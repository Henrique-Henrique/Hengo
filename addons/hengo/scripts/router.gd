...
static func get_current_route_v_cnodes() -> Array:
	if current_route:
		return (current_route.ref as WeakRef).get_ref().children.virtual_cnode_list \
			if (current_route.ref as WeakRef).get_ref() is HenVirtualCNode \
			else (current_route.ref as WeakRef).get_ref().virtual_cnode_list
...
static func change_route(_route: HenRouteData) -> void:
	...
	if current_route:
		var list: Array = (current_route.ref as WeakRef).get_ref().children.virtual_cnode_list \
			if (current_route.ref as WeakRef).get_ref() is HenVirtualCNode \
			else (current_route.ref as WeakRef).get_ref().virtual_cnode_list
