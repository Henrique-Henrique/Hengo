...
func configure_cnode_to_show(_cnode: HenCnode) -> void:
	...
	for connection: HenVCFlowConnectionData in flow.flow_connections_2:
		...
		connection.line_ref.from = weakref(from)
		connection.line_ref.to = weakref(to)
		connection.line_ref.from_idx = from.flow.flow_outputs.find(from.flow.get_flow_output(connection.from_id))
		connection.line_ref.to_idx = to.flow.flow_inputs.find(to.flow.get_flow_input(connection.to_id))

		# drawing inputs
		if connection.line_ref.to_pool_visible:
			var flow_idx = to.flow.flow_inputs.find(to.flow.get_flow_input(connection.to_id))
		...
		# drawing outputs
		if connection.line_ref.from_pool_visible:
			var flow_idx = from.flow.flow_outputs.find(from.flow.get_flow_output(connection.from_id))
...
func configure_cnode_to_hide(_cnode: HenCnode) -> void:
	...
	for connection: HenVCConnectionData in io.connections:
		...
		if not from.state.is_showing and not to.state.is_showing:
			connection.line_ref.visible = false
			connection.line_ref = null
