@tool
class_name HenActionApplyForce3D extends HenScriptMacroBase
	...
	return [
		{
			...
			type = 'Vector3',
			id = &'force',
			doc = 'The push to apply each frame, in units times mass per second squared.',
